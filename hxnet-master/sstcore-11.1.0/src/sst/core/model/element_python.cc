...
abortOnPyErr(uint32_t line, const char* file, const char* func, uint32_t exit_code, const char* format, ...)
{
    ...
    while ( ptb != nullptr ) {
        ...
        {
            PyCodeObject* code = PyFrame_GetCode(ptb->tb_frame);
            stream << "File \"" << PyUnicode_AsUTF8(code->co_filename) << "\", ";
            Py_DECREF(code);
#else
        {
        ...
        {
            PyCodeObject* code = PyFrame_GetCode(ptb->tb_frame);
            stream << PyUnicode_AsUTF8(code->co_name) << "\n";
            Py_DECREF(code);
#else
        {
...
SSTElementPythonModuleCode::load(void* parent_module)
{
    ...
    if ( parent != nullptr )
        PyModule_AddObject(pm, getFullModuleName().c_str(), module);
    else
        pm = module;
