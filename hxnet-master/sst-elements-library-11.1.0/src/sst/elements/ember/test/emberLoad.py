...
# SlimFly Hack
sys.path.append(
    "/scratch/tbonato/hxnet/sst-elements-library-11.1.0/src/sst/elements/ember/test/"
)
...
if detailedModelName:
    ...
    try:
        ...
        sys.exit(
            "Failed: could not import detailed model `{0}`".format(detailedModelName)
        )
    ...
    try:
        ...
        sys.exit(
            "Failed: could not import detailed model params `{0}`".format(
                detailedModelNameParms
            )
        )
...
if "torus" == netTopo:
    ...
    if usePlatNetConfig:
        ...
        topoInfo = HammingInfo(
            boardShape, netHostsPerRtr, globShape, fatTreeShape, routing_algo
        )
...
if rtrArb:
    print(
        "EMBER: network: topology={0} shape={1} arbitration={2}".format(
            netTopo, netShape, rtrArb
        )
    )
else:
...
if int(numNodes) > int(topoInfo.getNumNodes()):
    sys.exit(
        "need more nodes want "
        + str(numNodes)
        + ", have "
        + str(topoInfo.getNumNodes())
    )
...
if rndmPlacement and bgPercentage > 0:
    ...
    print(
        "EMBER: netAlloced {0}%, bg motifs {1}, mean {2} ns, stddev {3} ns, msgsize {4} bytes".format(
            int(bgPercentage * 100), bgMotifs, bgMean, bgStddev, bgMsgSize
        )
    )
    while count < bgMotifs:
        ...
        nidList = (
            "-nidList="
            + str(emptyNids[count * 2])
            + ","
            + str(emptyNids[count * 2 + 1])
        )
        motif = dict.copy(motifDefaults)
...
for a in params["network"]:
    ...
    if key in networkParams:
        print(
            "override networkParams {0}={1} with {2}".format(
                key, networkParams[key], value
            )
        )
    else:
...
for a in params["ember"]:
    ...
    if key in emberParams:
        print(
            "override emberParams {0}={1} with {2}".format(key, emberParams[key], value)
        )
    else:
...
for a in params["hermes"]:
    ...
    if key in hermesParams:
        print(
            "override hermesParams {0}={1} with {2}".format(
                key, hermesParams[key], value
            )
        )
    else:
...
for a in params["merlin"]:
    ...
    if key in sst.merlin._params:
        print(
            "override hermesParams {0}={1} with {2}".format(
                key, sst.merlin._params[key], value
            )
        )
    else:
...
print(
    "EMBER: network: BW={0} pktSize={1} flitSize={2}".format(
...
loadInfo = LoadInfo(
    topoInfo.getNumNodes(), topoInfo.getNicsPerNode(), baseNicParams, epParams
)

if len(loadFile) > 0:
    for jobid, nidlist, numCores, params, motifs in ParseLoadFile(
        loadFile, loadFileVars
    ):
...
if statsModuleName:
    ...
    try:
        ...
        sys.exit(
            "Failed: could not import statsistics module `{0}`".format(statsModuleName)
        )
