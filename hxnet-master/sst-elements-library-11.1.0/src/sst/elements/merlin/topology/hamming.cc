...
topo_hamming::topo_hamming(ComponentId_t cid, Params &params, int num_ports, int rtr_id, int num_vns) : Topology(cid),
                                                                                                        router_id(rtr_id),
                                                                                                        num_ports(num_ports),
                                                                                                        num_vns(num_vns)
{
    ...
    {
        // Helper: parse a comma-separated list of ints into a vector
        auto parse_csv = [](const std::string &s, std::vector<int> &out)
        {
            if (s.empty())
                return;
        ...
        parse_csv(params.find<std::string>("routing_table", ""), jf_routing_table);
        parse_csv(params.find<std::string>("row_ft_gateways", ""), jf_row_gateways);
        parse_csv(params.find<std::string>("col_ft_gateways", ""), jf_col_gateways);
        parse_csv(params.find<std::string>("dist_table", ""), jf_dist_table);
        DEBUG_PRINT(("--- Jellyfish mode: routing_table size=%zu, row_ft_port=%d, col_ft_port=%d, "
                     "row_gws=%zu, col_gws=%zu, dist_table=%zu ---\n",
...
std::vector<int> topo_hamming::getOutputPortFor(int fat_tree_destination)
{
    ...
    if (is_tree_edge_switch())
    {
        ...
        { // I am the edge destination switch
            output_ports.push_back(fat_tree_destination % down_ports_edge);
        }
        else
        { // I am the edge source switch
...
void topo_hamming::route_packet(int port, int vc, internal_router_event *ev)
{
    if (is_board_switch)
    {
...
bool topo_hamming::wrap_south(uint board_row_dest)
{
    // Shall I go south even if the destination is north?
    if (routing_algo == "min-adaptive-nogl")
    { // Never go on global links if src and dst are on the same board
        return false;
    }
    uint tree_hops;
    if (get_nrows() == 1)
...
bool topo_hamming::wrap_north(uint board_row_dest)
{
...
bool topo_hamming::wrap_east(uint board_col_dest)
{
...
bool topo_hamming::wrap_west(uint board_col_dest)
{
...
void topo_hamming::route_packet_mesh(int port, int vc, internal_router_event *ev)
{
...
void topo_hamming::route_packet_jellyfish(int port, int vc, internal_router_event *ev)
{
    int dest = ev->getDest();
    int src = ev->getSrc();
    ...
    // Case 2: Destination is on the same board -> shortest-path routing table,
    // optionally taking a fat-tree wrap shortcut when it is genuinely shorter.
    if (dest_board == my_board)
    {
        int next_port = jf_routing_table[dest_local];
        int out_vc = vc;
        // Intra-board fat-tree shortcut (mirrors the mesh wrap_* cost model)
        if (routing_algo != "min-adaptive-nogl" && !jf_dist_table.empty() && dest_local < jf_dist_table.size() && jf_dist_table[dest_local] > 2)
        {
            if (get_nrows() == 1 && jf_col_ft_port >= 0)
            {
    ...
    bool same_row = (row_dest == row_here);
    bool same_col = (col_dest == col_here);
    ...
    {
        // Different row AND column: choose the first dimension deterministically from
        // the destination (spreads diagonal traffic ~50/50 across row/col trees). After this tree traversal the packet becomes
        // non-diagonal and proceeds deterministically along the remaining dimension.
    ...
    // Fallback if gateway lists were not provided (older configs).
    if (target_edge < 0)
    {
        if (same_row && !same_col)
        {
    ...
    // Are we AT the chosen gateway and able to exit to the fat tree?
    if ((int)my_local == target_edge && ft_port >= 0)
    {
        DEBUG_PRINT(("[JF switch %d] At gateway, exiting to fat tree via port %d\n",
                     router_id, ft_port));
    ...
    {
        // Route toward the gateway node using the shortest-path table.
        int next_port = jf_routing_table[target_edge];
        DEBUG_PRINT(("[JF switch %d] Routing toward gateway %d via port %d\n",
    ...
    else
    {
        ...
        DEBUG_PRINT(("[JF switch %d] WARNING: no valid gateway found, delivering to NIC\n", router_id));
        ev->setNextPort(local_port_start);
        ev->setVC(vc);
    }
}
...
std::vector<int> topo_hamming::getUniqueBoardPosFromID(int switch_id)
{ // Switch ID == Dest ID
...
void topo_hamming::idToLocation(int run_id, int *location) const
{
...
void topo_hamming::parseDimString(const std::string &shape, int *output) const
{
    size_t start = 0;
    size_t end = 0;
    for (int i = 0; i < dimensions; i++)
...
int topo_hamming::get_dest_router(int dest_id) const
{
...
// dest_id is a host id
int topo_hamming::get_dest_local_port(int dest_id) const
{
    return local_port_start + (dest_id % num_local_ports);
}

int topo_hamming::choose_multipath(int start_port, int num_ports, int dest_dist)
{
    if (num_ports == 1)
    {
        return start_port;
...
uint topo_hamming::get_coord_board(uint addr)
{
    return addr / (board_shape[0] * board_shape[1]);
}

uint topo_hamming::get_coord_within_board(uint addr)
{
    return addr % (board_shape[0] * board_shape[1]);
}

uint topo_hamming::get_coord_board_col(uint addr)
{
    return get_coord_within_board(addr) % board_shape[1];
}

uint topo_hamming::get_coord_board_row(uint addr)
{
    return get_coord_within_board(addr) / board_shape[1];
}

uint topo_hamming::get_coord_col(uint addr)
{
    return get_coord_board(addr) % global_shape[1];
}

uint topo_hamming::get_coord_row(uint addr)
{
    return get_coord_board(addr) / global_shape[1];
}

int topo_hamming::getEndpointID(int port)
{
    if (!isHostPort(port))
        return -1;
...
void topo_hamming::setOutputBufferCreditArray(int const *array, int vcs)
{
    output_credits = array;
    num_vcs = vcs;
}

void topo_hamming::setOutputQueueLengthsArray(int const *array, int vcs)
{
