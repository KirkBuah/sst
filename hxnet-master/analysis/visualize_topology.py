...
from count_links import _routers_and_params, dims, dump, write_motif
...
def board_palette(n_boards):
    """Uniform RGB (0-255) for every board id (all boards the same color)."""
    color = (12, 77, 130)  # steel blue for all local boards
    return {b: color for b in range(n_boards)}


def model_options(board, glob, jelly, ft_nodes, motif):
    s = COMMON + (
        "--topo=hx --boardShape=%s --globalShape=%s --fatTreeShape=1:1,64 "
        % (board, glob)
    )
    if jelly:
        s += "--jellyfish "
    ...
    if include_nics:
        ...
        for l in d["links"]:
            ...
            if (
                ...
                and b in G
                and (
                    {G.nodes[a].get("role"), G.nodes[b].get("role")} == {"nic", "board"}
                )
            ):
...
def preview_png(G, path, title):
    ...
    # edges by type
    for lt, col, lw, alpha in (
        ("intra_board", "#1f77b4", 0.5, 0.6),
        ("fat_tree", "#d62728", 0.4, 0.35),
    ):
        segs = [
            (pos[u], pos[v])
            for u, v, dd in G.edges(data=True)
            if dd.get("link_type") == lt
        ]
...
def export_one(topo, jelly, args):
    ...
    d = dump(
        model_options(board, glob, jelly, args.ft_nodes, motif), wd, args.num_threads
    )

    G = build_graph(
        d, board, glob, args.gap, args.include_nics, args.scale, args.node_size
    )
    if args.single_board:
        # Keep one board's switches only; dropping the fat-tree nodes also drops
        # their incident (fat_tree) edges, leaving just the intra-board links.
        keep = [
            n
            for n, a in G.nodes(data=True)
            if a["role"] == "board" and a["board_id"] == args.board_id
        ]
        G = G.subgraph(keep).copy()
        for n in G.nodes:
            G.nodes[n]["size"] *= 1.8  # slightly bigger nodes for the single-board view
        base = os.path.join(OUT_DIR, "%s_board_%s" % (topo, board))
        title = "%s  (single board %s, %d switches)" % (topo, board, len(keep))
    else:
        base = os.path.join(OUT_DIR, "%s_%s_%s" % (topo, board, glob))
        title = "%s  (board %s, global %s, %d nodes)" % (topo, board, glob, n_nodes)
    ...
    n_intra = sum(
        1 for _, _, a in G.edges(data=True) if a.get("link_type") == "intra_board"
    )
    n_fttree = sum(
        1 for _, _, a in G.edges(data=True) if a.get("link_type") == "fat_tree"
    )
    print(
        "[%s] board switches=%d  fat-tree switches=%d  intra-board links=%d  fat-tree links=%d"
        % (topo, n_board, n_ft, n_intra, n_fttree)
    )
    print("       -> %s.graphml  /  %s.gexf" % (base, base))
    if args.preview:
        png = base + ".png"
        preview_png(G, png, title)
        print("       -> preview %s" % png)
    return G
...
def main():
    ap = ArgumentParser(
        description="Export HammingMesh fabric to GraphML/GEXF for Gephi"
    )
    ...
    ap.add_argument(
        ...
        type=int,
        default=None,
        help="spacing between boards (default ~ board/2)",
    )
    ap.add_argument(
        "--scale",
        type=float,
    ...
    ap.add_argument(
        "--single_board",
        action="store_true",
        help="render only one board (no other boards, no fat-tree switches)",
    )
    ap.add_argument(
        "--board_id", type=int, default=0, help="which board to keep with --single_board"
    )
    ap.add_argument(
        "--preview", action="store_true", help="also render a matplotlib PNG preview"
    )
    ...
    print(
        "Exporting HammingMesh fabric (board %s, global %s) -> %s\n"
        % (args.board_shape, args.global_shape, OUT_DIR)
    )
    todo = (
        [("mesh", False), ("jellyfish", True)]
        if args.topo == "both"
    ...
    print(
        "  - color is per board; fat-tree switches are dark squares; gateways are larger."
    )
    print(
        "  - Filters: by 'role' (board/fat_tree) or 'board_id'; Appearance: size by 'intra_degree'."
    )
    print(
        "  - Edge attribute 'link_type' separates intra_board vs fat_tree (partition/hide to compare)."
    )
