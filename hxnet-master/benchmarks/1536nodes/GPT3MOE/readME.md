...
This README explains how to reproduce the results of the GPT-3 MOE benchmark for the large topologies (1536 nodes).

## Usage

To launch the GPT-3 MOE jobs, please use the following script. By default all jobs are run locally using 8 threads.
```bash
...
## Results Only
It is also possible to use the already generated output files to generate the results. To do so, copy the `output` folder inside `results/GPT3-MOELarge/` to this folder (`benchmarks/largeTopology/GPT3MOE/`).
