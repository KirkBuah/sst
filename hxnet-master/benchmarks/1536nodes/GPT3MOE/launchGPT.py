...
def nid_generation(topo, size):
    list_other_nodes = []
    topo_nodes = range(0, 1536)
    if (topo == "dragonfly"):
        topo_nodes = range(0, 1632)

    size = int(size)
    px = 0
    if (size % 512 != 0 and size % 384 != 0):
    ...
    if (topo == "dragonfly"):
        ...
        if (size == 384):
            list_gpt_nodes = generate("4x4x8x8_24x16")
            px = 16
        elif (size == 512):
            list_gpt_nodes = generate("4x4x8x8_32x16")
            px = 16
        elif (size == 1536):
            list_gpt_nodes = generate("4x4x12x8")
        list_gpt_nodes = list_gpt_nodes.split(",")
        list_gpt_nodes = [int(i) for i in list_gpt_nodes]
        list_other_nodes = list(set(topo_nodes) - set(list_gpt_nodes))
    elif (topo == "hx2"):
        if (size == 384):
            list_gpt_nodes = generate("2x2x16x16_24x16")
            px = 16
        elif (size == 512):
            list_gpt_nodes = generate("2x2x16x16_32x16")
            px = 16
        elif (size == 1536):
            list_gpt_nodes = generate("2x2x24x16")
        list_gpt_nodes = list_gpt_nodes.split(",")
        list_gpt_nodes = [int(i) for i in list_gpt_nodes]
        list_other_nodes = list(set(topo_nodes) - set(list_gpt_nodes))
    if (topo == "torus" or topo == "hyperx" ):
        if (size == 384):
            list_gpt_nodes = generateTorus("32x32_24x16")
            list_gpt_nodes = list_gpt_nodes.split(",")
            list_gpt_nodes = [int(i) for i in list_gpt_nodes]
            list_other_nodes = list(set(topo_nodes) - set(list_gpt_nodes))
            px = 16
        elif (size == 512):
            list_gpt_nodes = generateTorus("32x32_32x16")
            list_gpt_nodes = list_gpt_nodes.split(",")
            list_gpt_nodes = [int(i) for i in list_gpt_nodes]
            list_other_nodes = list(set(topo_nodes) - set(list_gpt_nodes))
            px = 16
...
def all_reduce_type_from_topo(topo):
    if (topo == "hx2" or topo == "hx4" or topo == "torus"  or topo == "hyperx"):
        return "25D"
    else:
        return "05D"
...
def create_motif_load(name, motif_folder, topo, sizeJob):
    ...
    "[MOTIF] Init\n",
    "[MOTIF] GPT3MOE all_reduce_type={} px={}\n".format(all_reduce_type, px),
    "[MOTIF] Fini\n",
    "\n[JOB_ID] 11\n",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--topo", type=str, help="Topology to run", default="", choices=["hx4", "hx2", "hyperx", "fattree", "fattree50", "fattree75", "torus", "dragonfly"])
