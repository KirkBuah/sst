...
This README explains how to reproduce the results of the AllReduce benchmark for the small topologies (~1000 nodes). Note for this benchmark we also provide a `--small_run` option to run only a small subset of data points.
...
## Results Only
It is also possible to use the already generated output files to generate the plots. To do so, copy the `output` folder inside `results/AllReduceSmall/` to this folder (`benchmarks/smallTopology/AllReduce/`).
Once that has been done, just follow the previous instructions to generate the plots.