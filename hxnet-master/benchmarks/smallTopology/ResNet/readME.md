...
This README explains how to reproduce the results of the ResNet benchmark for the small topologies (~1000 nodes).

## Usage

To launch the ResNet jobs, please use the following script. By default all jobs are run locally using 8 threads.
```bash
python3 launchResNet.py
```
...
## Results Only
It is also possible to use the already generated output files to generate the results. To do so, copy the `output` folder inside `results/ResNetSmall/` to this folder (`benchmarks/smallTopology/ResNet/`).
