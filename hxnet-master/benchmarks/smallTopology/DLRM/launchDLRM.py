...
def nid_generation(topo, size):
    ...
    size = int(size)
    px = 0
    if (size != 128 and size != 256 and size != 1024):
    ...
    if (topo == "dragonfly"):
        ...
        if (size == 128):
            list_gpt_nodes = generate("4x4x8x8_16x8")
            px = 16
        elif (size == 512):
        ...
        if (size == 128):
            list_gpt_nodes = generate("2x2x16x16_16x8")
            px = 16
        elif (size == 512):
    ...
    if (topo == "torus" or topo == "hyperx"):
        if (size == 128):
            list_gpt_nodes = generateTorus("32x32_16x8")
            list_gpt_nodes = list_gpt_nodes.split(",")
            list_gpt_nodes = [int(i) for i in list_gpt_nodes]
            list_other_nodes = list(set(topo_nodes) - set(list_gpt_nodes))
            px = 16
