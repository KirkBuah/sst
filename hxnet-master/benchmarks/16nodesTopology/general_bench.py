...
generate_id_string_dragonfly = "[NID_LIST] generateNidList=generateNidListRange(0,16)\n"
generate_id_string_fattree_torus = "[NID_LIST] generateNidList=generateNidListRange(0,16)\n"
generate_id_string_hx4 = "[NID_LIST] generateNidList=generateNidListHx(2x2x2x2)\n"
generate_id_string_hx2 = "[NID_LIST] generateNidList=generateNidListHx(2x2x2x2)\n"
...
def run_slim(args, launch_string, size, name, topo):
    if (topo == "dragonfly" and args.nodes > 4):
        ...
        node_num = args.nodes
    slim_string = " time /scratch/2/t2hx/dep/openmpi/bin/mpirun -mca plm_rsh_no_tree_spawn 1 --map-by node -mca btl openib,self,sm -mca btl_openib_if_include mlx4_0 --hostfile /home/tbonato/{} -mca orte_base_help_aggregate 0  -np {} /scratch/tbonato/sstcore-11.1.0/libexec/sstsim.x".format(args.hostfile, node_num)
...
def run_sst(args, topo, size, name = ""):
    ...
    if (topo == "hx4"):
        launch_string = '--num_threads={} --model-options="--param="nic:module=merlin.reorderlinkcontrol" --topo=hx --boardShape=2x2 --globalShape=2x2 --fatTreeShape=1:1,64 --hostsPerRtr=1 --loadFile={}" {} > {}'.format(str(args.num_threads), location_motif, ember_load, output_result)
    elif (topo == "hx2"):
        launch_string = '--num_threads={} --model-options="--param="nic:module=merlin.reorderlinkcontrol" --topo=hx --boardShape=2x2 --globalShape=2x2 --fatTreeShape=1:1,64 --hostsPerRtr=1 --loadFile={}" {} > {}'.format(str(args.num_threads), location_motif, ember_load, output_result)
    elif (topo == "fattree"):
        launch_string = '--num_threads={} --model-options="--param="nic:module=merlin.reorderlinkcontrol" --topo=fattree --shape=4,4:4 --loadFile={}" {} > {}'.format(str(args.num_threads), location_motif, ember_load, output_result)
    elif (topo == "fattree50"):
        launch_string = '--num_threads={} --model-options="--param="nic:module=merlin.reorderlinkcontrol" --topo=fattree --shape=4,2:4 --loadFile={}" {} > {}'.format(str(args.num_threads), location_motif, ember_load, output_result)
    elif (topo == "fattree80"):
        launch_string = '--num_threads={} --model-options="--param="nic:module=merlin.reorderlinkcontrol" --topo=fattree --shape=4,1:4 --loadFile={}" {} > {}'.format(str(args.num_threads), location_motif, ember_load, output_result)
    elif (topo == "torus"):
        launch_string = '--num_threads={} --model-options="--param="nic:module=merlin.reorderlinkcontrol" --topo=torus --shape=4x4 --hostsPerRtr=1 --loadFile={}" {} > {}'.format(str(args.num_threads), location_motif, ember_load, output_result)
    elif (topo == "dragonfly"):
        # Special case for Dragonfly
        if args.num_threads > 8:
            args.num_threads = 8
        launch_string = '--num_threads={} --model-options="--param="nic:module=merlin.reorderlinkcontrol" --topo=dragonfly --shape=4:1:1:4 --loadFile={}" {} > {}'.format(str(args.num_threads), location_motif, ember_load, output_result)
    else:
