...
def main():
    ...
    bw_theo = []
    for data_size in x_data_theory:
        res = (4 * (abs(sqrt(64)) - 1) * (1*(40+20+20))) + (((abs(sqrt(64)) - 1) / (abs(sqrt(64)))) * (data_size * 8) * (1/400))
        bw_theo.append((data_size * 8) / res)
    x_data_plot = [element * 1 for element in x_data_theory]
    ...
    last_point_25d = 0
    for data_size in x_data_theory:
        #res = (2 * (64 - 1) * (1*(40+20+20))) + ((64-1)/64) * ((data_size / 2) * 8 / 64) * (1 / 400))
        res = (2 * (64 - 1) * (40+20+20)) + (((64 - 1) / 64) * ((data_size / 2 * 8) * (1 / 400)))
        #res = (4 * (sqrt(64) - 1) * (4*(40+20+20))) + (((sqrt(64) - 1) / (sqrt(64))) * (data_size * 8) * (1/400))
        bw_theo.append((data_size * 8) / res)
    x_data_plot = [element * 1 for element in x_data_theory]
    #bw_theo = [element * 4 for element in bw_theo]
    ...
    plt.title("Ring AllReduce - Small Topologies (64 nodes)")
    plt.legend()
    plt.show()
