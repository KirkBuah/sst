...
generate_id_string_dragonfly = "[NID_LIST] generateNidList=generateNidListRange(0,64)\n"
generate_id_string_fattree_torus = "[NID_LIST] generateNidList=generateNidListRange(0,64)\n"
...
def run_sst(args, topo, size, name = ""):
    ...
    if (topo == "hx4"):
        ...
        launch_string = '--num_threads={} --model-options="--param="nic:module=merlin.reorderlinkcontrol" --topo=hyperx --shape=8x8 --loadFile={}" {} > {}'.format(str(args.num_threads), location_motif, ember_load, output_result)
    elif (topo == "fattree"):
        launch_string = '--num_threads={} --model-options="--param="nic:module=merlin.reorderlinkcontrol" --topo=fattree --shape=8,8:8 --loadFile={}" {} > {}'.format(str(args.num_threads), location_motif, ember_load, output_result)
    elif (topo == "fattree50"):
        launch_string = '--num_threads={} --model-options="--param="nic:module=merlin.reorderlinkcontrol" --topo=fattree --shape=8,4:8 --loadFile={}" {} > {}'.format(str(args.num_threads), location_motif, ember_load, output_result)
    elif (topo == "fattree80"):
        launch_string = '--num_threads={} --model-options="--param="nic:module=merlin.reorderlinkcontrol" --topo=fattree --shape=8,2:8 --loadFile={}" {} > {}'.format(str(args.num_threads), location_motif, ember_load, output_result)
    elif (topo == "torus"):
        ...
        if args.num_threads > 8:
            args.num_threads = 8
        launch_string = '--num_threads={} --model-options="--param="nic:module=merlin.reorderlinkcontrol" --topo=dragonfly --shape=2:4:4:8 --loadFile={}" {} > {}'.format(str(args.num_threads), location_motif, ember_load, output_result)
    else:
