...
Usage:
    python3 launchAllReduce.py --board_shape 4x4 --global_shape 8x8 --small_run
    python3 launchAllReduce.py --board_shape 4x4 --global_shape 8x8 --jellyfish --small_run
    python3 launchAllReduce.py --board_shape 4x4 --global_shape 8x8 --ring_variant ring05d
...
large_sizes = [2**18, 2**20, 2**22, 2**24, 2**26, 2**28, 2**30]
small_sizes = [2**18, 2**22, 2**26]
...
def run_simulation(args, msg_size, motif_name):
    ...
    if args.env in ("cluster", "slurm", "ault", "slimfly"):
        run_cluster(args, launch_string, msg_size, "RingAllReduce", topo_name)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--small_run", action="store_true",
                        help="Run only 3 sizes instead of 7")
