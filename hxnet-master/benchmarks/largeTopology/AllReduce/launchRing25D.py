...
sizes = [2**18, 2**20, 2**22, 2**24, 2**26, 2**28, 2**30]
...
def create_motif_load(name):
    ...
    "[MOTIF] Init\n",
    "[MOTIF] RingAllreduce25D count=512 aggregation_cost_ns=0 blocking=true\n",
    "[MOTIF] Fini"]
            ...
            if (args.size == 0):
                with open(location_motif, 'r') as f:
                    for line in f:
                        ...
                        if (topo == "dragonfly"):
                            generating_id_string = "[NID_LIST] generateNidList=generateNidListRange(0,16320)\n"
                        elif (topo != "hx4" and topo != "hx2"):
                            generating_id_string = "[NID_LIST] generateNidList=generateNidListRange(0,16384)\n"
                        elif (topo == "hx4"):
                            generating_id_string = "[NID_LIST] generateNidList=generateNidListHx(4x4x32x32)\n"
                        elif (topo == "hx2"):
                            generating_id_string = "[NID_LIST] generateNidList=generateNidListHx(2x2x64x64)\n"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--topo", type=str, help="Topology to run", default="", choices=["hx4", "hx2", "hyperx", "fattree", "fattree50", "fattree80", "fattree75", "torus", "dragonfly"])
