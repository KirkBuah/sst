...
from generateNidListTorus import generate as generateTorus
...
def nid_generation(topo, size):
    list_other_nodes = []
    topo_nodes = range(0, 16384)
    if (topo == "dragonfly"):
        topo_nodes = range(0, 16320)

    size = int(size)
    px = 0
    if (size != 256 and size != 512 and size != 1024 and size != 2048):
    ...
    if (topo == "dragonfly"):
        ...
        if (size == 256):
            list_gpt_nodes = generate("4x4x32x32_16x16")
            px = 16
        elif (size == 512):
            list_gpt_nodes = generate("4x4x32x32_32x16")
            px = 16
        elif (size == 1024):
            list_gpt_nodes = generate("4x4x32x32_32x32")
            px = 32
        elif (size == 2048):
            list_gpt_nodes = generate("4x4x32x32_64x32")
            px = 32
        ...
        if (size == 256):
            list_gpt_nodes = generate("2x2x64x64_16x16")
            px = 16
        elif (size == 512):
            list_gpt_nodes = generate("2x2x64x64_32x16")
            px = 16
        elif (size == 1024):
            list_gpt_nodes = generate("2x2x64x64_32x32")
            px = 32
        elif (size == 2048):
            list_gpt_nodes = generate("2x2x64x64_64x32")
            px = 32
        list_gpt_nodes = list_gpt_nodes.split(",")
        list_gpt_nodes = [int(i) for i in list_gpt_nodes]
        list_other_nodes = list(set(topo_nodes) - set(list_gpt_nodes))
    if (topo == "torus"):
        if (size == 256):
            list_gpt_nodes = generateTorus("128x128_16x16")
            list_gpt_nodes = list_gpt_nodes.split(",")
            list_gpt_nodes = [int(i) for i in list_gpt_nodes]
            list_other_nodes = list(set(topo_nodes) - set(list_gpt_nodes))
            px = 16
        elif (size == 512):
            list_gpt_nodes = generateTorus("128x128_32x16")
            list_gpt_nodes = list_gpt_nodes.split(",")
            list_gpt_nodes = [int(i) for i in list_gpt_nodes]
            list_other_nodes = list(set(topo_nodes) - set(list_gpt_nodes))
            px = 16
        elif (size == 1024):
            list_gpt_nodes = generateTorus("128x128_32x32")
            list_gpt_nodes = list_gpt_nodes.split(",")
            list_gpt_nodes = [int(i) for i in list_gpt_nodes]
            list_other_nodes = list(set(topo_nodes) - set(list_gpt_nodes))
            px = 32
        elif (size == 2048):
            list_gpt_nodes = generateTorus("128x128_64x32")
            list_gpt_nodes = list_gpt_nodes.split(",")
            list_gpt_nodes = [int(i) for i in list_gpt_nodes]
            list_other_nodes = list(set(topo_nodes) - set(list_gpt_nodes))
            px = 32
    ...
    return list_gpt_nodes, list_other_nodes, px
...
def create_motif_load(name, motif_folder, topo, sizeJob):
    ...
    list_gpt_nid, list_null_nid, px = nid_generation(topo, sizeJob)
    ...
    "[MOTIF] Init\n",
    "[MOTIF] ResNet152 all_reduce_type={} px={}\n".format(all_reduce_type, px),
    "[MOTIF] Fini\n",
    "\n[JOB_ID] 11\n",
    ...
    if (sizeJob >= 16320):
        motif_content = motif_content[:len(motif_content)-5]
